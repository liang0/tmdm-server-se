...
public class StorageFullTextTest extends StorageTestCase {
    ...
    private static void populateData() throws Exception {
        ...
        allRecords.add(factory.read(repository, contract, "<Contract><id>1</id><comment>1</comment><detail xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\"ContractDetailType\"><code>1</code></detail><detail xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\"ContractDetailType\"><code>1</code></detail><detail xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\"ContractDetailSubType\"><code>1</code><features><actor>1</actor><vendor>1</vendor></features></detail><enumEle>pending</enumEle></Contract>"));
        try {
            storage.begin();
    ...
    public void testInheritSearch() {
        ...
        UserQueryBuilder qb = from(contract).where(fullText("1"));
        for (TypedExpression field : fields) {
            qb.select(field);
        }
        qb.select(contract.getField("id"));
        qb.select(contract.getField("comment"));
        qb.select(contract.getField("enumEle"));
